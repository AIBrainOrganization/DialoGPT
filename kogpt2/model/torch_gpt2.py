...
class Attention(nn.Module):
  def __init__(self, nx, n_ctx, config, scale=False):
    ...
    n_state_p = config.n_position_dim
    n_state_e = config.n_emotion_dim
    n_state = nx  # in Attention: n_state=768 (nx=n_embd)
    # [switch nx => n_state from Block to Attention to keep identical to TF implem]
    n_state_a = n_state + n_state_e
    # print("n_state_a: ", n_state_a, config.n_head)
    assert n_state_a % config.n_head == 0
    self.register_buffer("bias", torch.tril(
        torch.ones(n_ctx, n_ctx)).view(1, 1, n_ctx, n_ctx))
    self.n_head = config.n_head
    self.split_size = n_state
    self.split_size_e = n_state_e 
    self.scale = scale
    self.n_state = n_state
    self.n_state_e = n_state_e
    self.n_state_a = n_state_a

    self.c_attn = Conv1D(n_state * 3, nx)
    self.c_attn_e = Conv1D(n_state_e * 3, n_state_e)
    self.c_proj = Conv1D(nx, n_state_a)
    ...
    pos_dim = config.n_position_dim
    position_embedding_q = np.array([[1 if pos == j else 0 for j in range(pos_dim)] for pos in range(pos_dim)] )
    position_embedding_k = np.array([[math.exp(-(pos -j) ** 2 / (2 * sigma ** 2)) for j in range(pos_dim)] for pos in range(pos_dim)])
    self.position_embedding_query = torch.FloatTensor(position_embedding_q).cuda()
    self.position_embedding_key = torch.FloatTensor(position_embedding_k).cuda()
    # self.position_embedding_key = torch.transpose(self.position_embedding_key,0,1) #symmetric
    self.position_attn = torch.matmul(self.position_embedding_query, self.position_embedding_key).cuda()
  ...
  def _attn(self, q, k, v, attention_mask=None, head_mask=None):
    ...
    # w_size = w.shape[-1]
    pos_attn = self.position_attn.repeat(w.size(-4), w.size(-3), 1, 1)
    pos_attn = pos_attn.cuda()
    # pos_attn = pos_attn / (float(self.hid_dim) ** 0.5) if self.scale else pos_attn
    pos_beta = 10
  ...
  def forward(self, x, layer_past=None, attention_mask=None, head_mask=None):
    ...
    x_e = x[:,:,self.n_state:]
    x_w = self.c_attn(x_w)
    x_e = self.c_attn_e(x_e)
    query_w, key_w, value_w = x_w.split(self.split_size, dim=2)
    query_e, key_e, value_e = x_e.split(self.split_size_e, dim=2)
    
    query_cat = torch.cat((query_w, query_e), 2)
    key_cat = torch.cat((key_w, key_e), 2)
    value_cat = torch.cat((value_w, value_e), 2)

    query = self.split_heads(query_cat)
    key = self.split_heads(key_cat, k=True)
    value = self.split_heads(value_cat)
...
class Block(nn.Module):
  def __init__(self, n_ctx, config, scale=False):
    super().__init__()
    self.nx_e = config.n_embd + config.n_emotion_dim
    self.nx = config.n_embd
    nx_e = self.nx_e
    nx = self.nx
    self.ln_1 = nn.LayerNorm(nx_e, eps=config.layer_norm_epsilon)
...
class GPT2Model(GPT2PreTrainedModel):
  ...
  def __init__(self, config):
    ...
    self.ln_f = nn.LayerNorm(config.n_embd + config.n_emotion_dim, eps=config.layer_norm_epsilon) # +config.n_emotion_dim
  ...
  def forward(
    ...
    for i, (block, layer_past) in enumerate(zip(self.h, past)):
      ...
      #print("layer2~layer12_hidden_state: ", hidden_states.size)
      hidden_states = torch.cat((hidden_states, emotion_embeds), 2)
      #print("hidden_states_547: ", hidden_states.shape)
      #print("layer2~layer12_hidden_state_cat: ", hidden_states.size)
      if self.output_past:
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
  ...
  def __init__(self, config):
    ...
    self.init_weights()
    self.n_word_dim = config.n_word_dim
  ...
  def forward(
    ...
    #print("outputs_hidden_states: ", hidden_states)
    hidden_states = hidden_states[:,:,:self.n_word_dim]
