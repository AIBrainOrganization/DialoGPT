...
def convert_examples_to_features_dynamic(examples, tokenizer, vocab,
                                         max_seq_length=512):
  """
  do not pad
  """
  def featurize(example):
    ...
    context_id = vocab[tokenizer(example.context)]
    end_of_text_id = vocab[vocab.eos_token]

    # response is provided in example
    response_id = vocab[tokenizer(example.response)]
...
class DynamicBatchingLoader(object):
  ...
  def __init__(self, corpus_file, tokenizer, vocab, normalize_data,
      ...
      with open(self.corpus, 'r', encoding="utf-8") as corpus:
        ...
        while True:
          ...
          features = convert_examples_to_features_dynamic(
              examples, self.toker, self.vocab, self.max_seq_length)
