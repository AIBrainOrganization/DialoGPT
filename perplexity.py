...
def eval_model_loss(model, tokenizer, eval_dataloader, args, vocab):
  ...
  with torch.no_grad():
    for step, batch in enumerate(tqdm(eval_dataloader, total=136395)):
      ...
      if args.no_token_id:
        token_ids = None
      inputs.append(
          decode(input_ids.tolist()[0], vocab, skip_special_tokens=False))
      loss, ppl, _ = model(input_ids,
                           position_ids=position_ids,
                           token_type_ids=token_ids,
                           labels=label_ids)
      tot_ppl.append(ppl.sum().item())
  pd.DataFrame((inputs, tot_ppl)).T.to_excel('perplexities.xlsx',
                                             header=False,
                                             index=False)
  return np.mean(tot_ppl)


if args.local_rank == -1 or get_rank() == 0:
  eval_ppl = eval_model_loss(model, enc, eval_dataloader_loss, args, vocab)
  print(f'{eval_ppl}', file=eval_logger)
