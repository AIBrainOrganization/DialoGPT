...
parser.add_argument('--data', type=str, default='koMulti',
                    help='koMulti')
dargs = parser.parse_args()

assert dargs.data == 'koMulti', 'The specified data option is not support!'
...
if os.path.exists(MODEL_FOLDER):
  print(
      f'Found existing models folder at {MODEL_FOLDER}, skip creating a new one!')
...
# target_folder = download_model(
#     model_size='small', dataset='multiref', from_scratch=False)
# logger.info('Done!\n')
...
#   ret = sp.run(cmd.split(' '), stdout=sp.PIPE,
#                stderr=sp.STDOUT, cwd=DATA_FOLDER)
...
# else:
#   raise ValueError(
#       'you need to implement your own data type, or use either dummy, small, or full')
#
# if ret.returncode != 0:
...
if os.path.isdir(data_db):
  ...
  print(cmd)
  ret = sp.run([PYTHON_EXE] + cmd.split(' '), stdout=sp.PIPE,
               stderr=sp.STDOUT, cwd=PROJECT_FOLDER)
  if ret.returncode != 0:
    print(f'error occurred, {ret.stdout}')
    sys.exit(ret.returncode)
logger.info('Done!\n')
...
args = [
    ...
    '--eval_input_file', './data/valid.tsv',   # dummy test data
...
with open('./output.log', 'wb') as f:
  process = sp.Popen([PYTHON_EXE] + train_cmd.split(' '),
                     stdout=sp.PIPE, stderr=sp.STDOUT, cwd=PROJECT_FOLDER)
